...
export default class CevalCtrl {
  ...
  init(opts: CevalOpts): void {
    ...
    const pos = this.opts.initialFen
      ? parseFen(this.opts.initialFen).chain((setup) =>
          setupPosition(this.rules, setup)
        )
  ...
  onEmit: (ev: Tree.LocalEval, work: Work) => void = throttle(
    200,
    (ev: Tree.LocalEval, work: Work) => {
      this.sortPvsInPlace(
        ev.pvs,
      ...
      if (
        ...
        return;
      const showingNode =
        this.lastStarted.steps[this.lastStarted.steps.length - 1];
      const byMovetime =
        'movetime' in this.search.by && this.search.by.movetime;
      if (
        byMovetime &&
  ...
  private doStart = (
    ...
    if (
      ...
      ((threatMode ? step.threat : step.ceval)?.millis ?? 0) >=
        this.search.by.movetime
    ) {
  ...
  get search(): Search {
    const s = {
      ...
      by: {
        movetime: Math.min(this.storedMovetime(), this.engines.maxMovetime),
      },
      ...this.customSearch,
    };
    if (this.isDeeper() || (this.isInfinite && !this.customSearch))
      s.by = { depth: 99 };
    return s;
  }
  ...
  get canGoDeeper(): boolean {
    return (
      this.state !== CevalState.Computing && (this.curEval?.depth ?? 0) < 99
    );
  }
  ...
  setThreads = (threads: number): void =>
    storage.set('ceval.threads', threads.toString());
  ...
  setHashSize = (hash: number): void =>
    storage.set('ceval.hash-size', hash.toString());
