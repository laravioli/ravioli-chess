...
var Sf167Web = (() => {
  ...
  return async function (moduleArg = {}) {
    ...
    // include: /home/victor/Bureau/Code/stockfish/lila-stockfish-web/src/initModule.js
    /**
     ...
     * @type {function(string):void}
     */ if (!Module['onError'])
      Module['onError'] = (data) => console.error(data);

    /** @private, @param {number=} index */ Module['getRecommendedNnue'] = (
      index = 0
    ) => UTF8ToString(_getRecommendedNnue(index));

    /** @private, @param {Uint8Array} buf, @param {number=} index */ Module[
      'setNnueBuffer'
    ] = function (buf, index = 0) {
      ...
      // deallocated in src/wasm/glue.cpp
      if (!heapBuf)
        throw new Error(`could not allocate ${buf.byteLength} bytes`);
      Module['HEAPU8'].set(buf, heapBuf);
      _setNnueBuffer(heapBuf, buf.byteLength, index);
    };

    /** @private, @param {string} command */ Module['uci'] = function (
      command
    ) {
    ...
    /** @private, @param {string} data */ Module['print'] = (data) =>
      Module['listen']?.(data);

    /** @private, @param {string} data */ Module['printErr'] = (data) =>
      Module['onError']?.(data);

    // end include: /home/victor/Bureau/Code/stockfish/lila-stockfish-web/src/initModule.js
...
/** not-t@type {!BigUint64Array} */ HEAPU64,
    ...
    /** @param {number|boolean=} isFloat */ function getSafeHeapType(
      bytes,
      isFloat
    ) {
      switch (bytes) {
    ...
    /** @param {number|boolean=} isFloat */ function SAFE_HEAP_STORE(
      dest,
      value,
      bytes,
      isFloat
    ...
    /** @param {number|boolean=} isFloat */ function SAFE_HEAP_LOAD(
      dest,
      bytes,
      unsigned,
      isFloat
    ...
    function SAFE_FT_MASK(value, mask) {
      ...
      if (ret !== value) {
        abort(
          `Function table mask error: function pointer is ${value} which is masked by ${mask}, the likely cause of this is that the function pointer is being called by the wrong type.`
        );
      }
      return ret;
    }
    ...
    function findWasmBinary() {
      if (Module['locateFile']) {
        return locateFile('sf16-7.wasm');
      }
      // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
      return new URL('sf16-7.wasm', import.meta.url).href;
    }
    ...
    var PThread = {
      ...
      allocateUnusedWorker() {
        ...
        worker = new Worker(new URL('sf16-7.js', import.meta.url), {
          type: 'module',
          // This is the way that we signal to the Web Worker that it is hosting
          // a pthread.
          name: 'em-pthread-' + PThread.nextWorkerID,
    ...
    var __Z10js_getlinev = (Module['__Z10js_getlinev'] = createExportWrapper(
      '_Z10js_getlinev',
      1
    ));
    ...
    var _uci = (Module['_uci'] = createExportWrapper('uci', 1));

    var _setNnueBuffer = (Module['_setNnueBuffer'] = createExportWrapper(
      'setNnueBuffer',
      3
    ));

    var _getRecommendedNnue = (Module['_getRecommendedNnue'] =
      createExportWrapper('getRecommendedNnue', 1));
...
(() => {
  // Create a small, never-async wrapper around Sf167Web which
  // checks for callers incorrectly using it with `new`.
  var real_Sf167Web = Sf167Web;
  Sf167Web = function (arg) {
    if (new.target)
      throw new Error('Sf167Web() should not be called with `new Sf167Web()`');
    return real_Sf167Web(arg);
  };
})();
