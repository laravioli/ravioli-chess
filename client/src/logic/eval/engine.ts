...
export class StockfishWebEngine implements CevalEngine {
  ...
  async boot(): Promise<void> {
    const [pathVersion, root, js] = [
      this.info.assets.version,
      this.info.assets.root,
      this.info.assets.js,
    ];
    const makeModule = await import(
      site.asset.url(`${root}/${js}`, { pathVersion, documentOrigin: true })
    );
    const module: StockfishWeb = await makeModule.default({
      ...
      locateFile: (file: string) =>
        site.asset.url(`${root}/${file}`, { pathVersion }),
    });
      ...
      if (this.info.variants?.length === 1) {
        ...
        module.uci(
          `setoption name UCI_Variant value ${
            model === 'threecheck' ? '3check' : model
          }`
        );
      }
      this.store = await objectStorage<Uint8Array>({
        store: 'nnue',
        db: 'nnue--db',
      }).catch(() => undefined);
