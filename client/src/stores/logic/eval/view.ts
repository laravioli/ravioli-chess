...
function localEvalNodes(
  ...
  if (evs.client.cloud && !ceval.isComputing)
    t.push(
      h('span.cloud', { attrs: { title: i18n.site.cloudAnalysis } }, 'Cloud')
    );
  if (ceval.isInfinite)
    t.push(
      h('span.infinite', { attrs: { title: i18n.site.infiniteAnalysis } }, '∞')
    );
...
function localInfo(ctrl: ParentCtrl, ev?: Tree.ClientEval | false): EvalInfo {
  ...
  const ceval = ctrl.getCeval();
  info.depthText =
    i18n.site.depthX(ev.depth || 0) +
    (ceval.isDeeper() || ceval.isInfinite ? '/99' : '');
  ...
  if (knps > 0) {
    info.npsText = `${
      knps > 1000
        ? (knps / 1000).toFixed(knps > 10000 ? 0 : 1) + ' Mn/s'
        : Math.round(knps) + ' kn/s'
    }`;
    info.knps = knps;
  }
  return info;
...
function threatButton(ctrl: ParentCtrl): VNode | null {
  if (
    ctrl.getCeval().download ||
    (ctrl.disableThreatMode && ctrl.disableThreatMode())
  )
    return null;
  ...
  return engine
    ? [
        ...
        ctrl.engines.isExternalEngineInfo(engine)
          ...
          ? h(
              'span.technology.good',
              { attrs: { title: 'Multi-threaded WebAssembly' } },
              engine.tech
            )
          : engine.requires.includes('wasm')
          ? h(
              'span.technology',
              { attrs: { title: 'Single-threaded WebAssembly' } },
              engine.tech
            )
          : h(
              'span.technology',
              { attrs: { title: 'Single-threaded JavaScript' } },
              engine.tech
            ),
      ]
    : [];
...
export function renderCeval(ctrl: ParentCtrl): LooseVNodes {
  ...
  if (evs.client) {
    ...
    else if (ceval.isDeeper() || ceval.isInfinite)
      percent = Math.min(100, (100 * evs.client.depth) / 99);
    else if ('movetime' in search.by)
      percent = Math.min(
        100,
        (100 * ((threat ?? evs.client)?.millis ?? 0)) / search.by.movetime
      );
    else if ('depth' in search.by)
      percent = Math.min(100, (100 * evs.client.depth) / search.by.depth);
    else if ('nodes' in search.by)
      percent = Math.min(100, (100 * evs.client.nodes) / search.by.nodes);
  }
  ...
  if (download)
    percent = Math.min(
      100,
      Math.round((100 * download.bytes) / download.total)
    );
  else if (ceval.search.indeterminate || (percent > 0 && !ceval.isComputing))
    percent = 100;
  ...
  const switchButton: VNode | false =
    ...
    h(
      'div.switch',
      { attrs: { title: i18n.site.toggleLocalEvaluation + ' (L)' } },
      [
        h('input#analyse-toggle-ceval.cmn-toggle.cmn-toggle--subtle', {
          ...
          hook: onInsert((el: HTMLInputElement) => {
            el.addEventListener(
              'keydown',
              (e) => (e.key === 'Enter' || e.key === ' ') && ctrl.toggleCeval()
            );
      ...
      hook: {
        insert: (vnode) => {
          ...
          el.addEventListener('mouseout', () =>
            ctrl.getCeval().setHovering(getElFen(el))
          );
...
function renderPv(
  ...
  if (pv) {
    ...
    if (multiPv > 1) {
      children.push(
        h('strong', defined(pv.mate) ? '#' + pv.mate : renderEval(pv.cp!))
      );
    }
    if (pos) {
      children.push(
        ...renderPvMoves(pos.clone(), pv.moves.slice(0, MAX_NUM_MOVES))
      );
    }
  }
...
function renderPvMoves(pos: Position, pv: Uci[]): VNode[] {
  ...
  for (let i = 0; i < pv.length; i++) {
    ...
    vnodes.push(
      h(
        'span.pv-san',
        { key, attrs: { 'data-move-index': i, 'data-board': `${fen}|${uci}` } },
        san
      )
    );
  }
...
function renderPvBoard(ctrl: ParentCtrl): VNode | undefined {
  ...
  const cgVNode = h('div.cg-wrap.is2d', {
    hook: {
      insert: (vnode: any) =>
        (vnode.elm._cg = makeChessground(vnode.elm, cgConfig)),
...
function loadingText(ctrl: ParentCtrl): string {
  ...
  if (d && d.total)
    return `Downloaded ${Math.round(
      (d.bytes * 100) / d.total
    )}% of ${Math.round(d.total / 1000 / 1000)}MB`;
